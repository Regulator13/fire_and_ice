/// @description draw buttons
    ...
    for (var i = 0; i < ds_list_size(buttons); i++) {
        ...
        if (button.action == "value" || button.action == "valueAction") {
            ...
            draw_sprite(button.sprite_index, button.image_index, button.x, button.y)
            draw_set_halign(fa_center);
            if (button.title == "Default") draw_text(button.x, button.y, string_hash_to_newline(button.action));
            else draw_text(button.x, button.y, string_hash_to_newline(button.title));
            }
        }
    //draw selector
...
if (state == STATE_LOBBY) {
    ...
    if (global.online) {
        ...
        for (var i = 0; i < playerAmount; i += attributes) {
            ...
            draw_text(48+sw, 40 + bh*i / attributes, string_hash_to_newline("Team " + string(ds_list_find_value(serverData, i))));
            draw_text(128+sw, 40 + bh*i / attributes, string_hash_to_newline("Ready: " + string(ds_list_find_value(serverData, i + 1))));
            draw_text(240+sw, 40 + bh*i / attributes, string_hash_to_newline("Name: " + string(ds_list_find_value(serverData, i + 2))));
            draw_text(400+2*sw, 40 + bh*i / attributes, string_hash_to_newline("Character: " + ds_list_find_value(serverData, i + 3)));
...
if (state == STATE_PATHS) {
    ...
    if (global.online) {
        ...
        for (var i = offset; i < pathAmount; i += attributes) {
            ...
            draw_text(48, 40 + 32*drawOffset, string_hash_to_newline(ds_list_find_value(serverData, i)));
            draw_text(304, 40 + 32*drawOffset, string_hash_to_newline("High Score: " + string(ds_list_find_value(serverData, i + 1))));
            draw_text(480, 40 + 32*drawOffset, string_hash_to_newline("Length: " + string(ds_list_find_value(serverData, i + 2))));
            drawOffset++;
            }
    ...
    if (global.online) {
        ...
        for (var i = 1; i < teamAmount; i++){
            ...
            if (teamExists) {
                ...
                //draw nickname
                tb = 2 //text buffer
                draw_text(cx+HUDwidth/2, cy+tb, string_hash_to_newline(ds_list_find_value(serverData, index++))); // get nickname
                //draw score and lives
                draw_set_halign(fa_left);
                
                draw_text(cx+tb, cy+tb*3+12, string_hash_to_newline("SC: " + ds_list_find_value(serverData, index++))); // get score
                draw_text(cx+tb+72, cy+tb*3+12, string_hash_to_newline("LV: " + ds_list_find_value(serverData, index++))); // get level
                draw_set_halign(fa_center);
                draw_text(cx+HUDwidth/2+tb, cy+tb*3+24, string_hash_to_newline("Lives: " + ds_list_find_value(serverData, index++))); // get lives
    ...
    if (state == STATE_SCORE) {
        ...
        for (var i = 1; i < teamAmount; i++){
            ...
            if (teamExists) {
                ...
                // draw score
                draw_set_font(fnt_score);
                draw_text(room_width/2, 200 + 26*sy++, string_hash_to_newline(nickname + " Score: " + teamScore + " Lives: " + teamLives));
                ...
                draw_text(cx+tb, cy+tb*3+12, string_hash_to_newline("SC: " + teamScore)); // get score
                draw_text(cx+tb+72, cy+tb*3+12, string_hash_to_newline("LV: " + teamLevel)); // get level
                draw_set_halign(fa_center);
                draw_text(cx+HUDwidth/2+tb, cy+tb*3+24, string_hash_to_newline("Lives: " + teamLives)); // get lives
