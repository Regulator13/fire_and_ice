/// @function scr_platform_generate(dx, dy, range)
...
switch(argument2) {
    ...
    case 0: //five long platform
        for (k = 0; k < 5; k += 1) {
            ...
            if (place_empty(dx,dy)) {
                instance_create_layer(dx, dy, "lay_instances", obj_block_big);
                }
            }
	...
	case 1: //five long 'u'
		if (place_empty(dx,dy-gridSize)){
			instance_create_layer(dx, dy-gridSize, "lay_instances", obj_block_big);
		...
		for (k = 0; k < 5; k += 1) {
			dx = sx+gridSize*k;
			if (place_empty(dx,dy)) instance_create_layer(dx, dy, "lay_instances", obj_block_big);
		}
			
		if (place_empty(dx,dy-gridSize)) instance_create_layer(dx, dy-gridSize, "lay_instances", obj_block_big);
	...
	case 4: //five long 'u'
		if (place_empty(dx,dy-gridSize)) instance_create_layer(dx, dy-gridSize/2, "lay_instances", obj_block_station);
	...
	case 5: //three long with block
		...
		if (place_empty(dx,dy-gridSize)) with(instance_create_layer(dx, dy-gridSize/2, "lay_instances", obj_block_station)) sticky = true;
